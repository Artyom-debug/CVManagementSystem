...
public sealed record RemovePositionCommand(Guid PositionId, int Version) : IRequest<Result>;
...
internal sealed class RemovePositionCommandHandler : IRequestHandler<RemovePositionCommand, Result>
{
    ...
    public async Task<Result> Handle(RemovePositionCommand request, CancellationToken cancellationToken)
    {
        ...
        var hasCVs = await _context.CVs
            .AnyAsync(cv => cv.PositionId == position.Id, cancellationToken);

        if (hasCVs)
        {
            return Result.Failure("A position with existing CVs cannot be deleted.");
        }
        ...
        {
            return Result.Failure("The position cannot be deleted because it is already used by a CV.");
        }
