...
internal sealed class GetDetailedPositionQueryHandler : IRequestHandler<GetDetailedPositionQuery, DetailedPositionDto>
{
    ...
    public GetDetailedPositionQueryHandler(IApplicationDbContext context, ICacheService cache)
    {
        _context = context;
        _cache = cache;
    }
    ...
    public async Task<DetailedPositionDto> Handle(GetDetailedPositionQuery request, CancellationToken cancellationToken)
    {
        ...
        var result = new DetailedPositionDto(position.Id, position.Version, position.Name, position.Description, position.MaxProjectCount, position.IsPublic, position.Tags.OrderBy(tag => tag.Name).Select(tag => tag.Name).ToList(), position.PositionAttributes.OrderBy(item => item.DisplayOrder).Select(item => new PositionAttributeDto(item.DisplayOrder, new DetailedAttributeDto(item.Attribute!.Id, item.Attribute.Version, item.Attribute.Name, item.Attribute.Description, item.Attribute.Type, item.Attribute.Category, item.Attribute.IsSystem, item.Attribute.Options.OrderBy(option => option.Option).Select(option => new AttributeOptionDto(option.Id, option.Option)).ToList()))).ToList(), position.AccessRules.Select(rule => new PositionAccessRuleDto(rule.AttributeId, rule.AttributeType, rule.Operator, rule.Value.StringValue, rule.Value.NumericValue, rule.Value.DateValue, rule.Value.PeriodStart, rule.Value.PeriodEnd, rule.Value.BooleanValue, rule.Value.DropdownOptionId)).ToList(), position.DiscussionPosts.OrderBy(post => post.CreatedAt).Select(post => new DiscussionPostDto(post.Id, post.AuthorId, post.Content, post.CreatedAt)).ToList());
